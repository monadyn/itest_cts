...
FROM bitten_build AS build
 LEFT OUTER JOIN CTSTestSheet AS report ON (report.build=build.id)
 LEFT OUTER JOIN CTSTestSheet_item AS item_status
...
FROM CTSTestSheet AS report
 LEFT OUTER JOIN CTSTestSheet_item AS item_fixture
 ...
 LEFT OUTER JOIN CTSTestSheet_item AS item_file
  ON (item_file.report=report.id AND item_file.item=item_fixture.item AND
      item_file.name='file')
 LEFT OUTER JOIN CTSTestSheet_item AS item_success
 ...
 LEFT OUTER JOIN CTSTestSheet_item AS item_ignore
 ...
 LEFT OUTER JOIN CTSTestSheet_item AS item_failure
 ...
 LEFT OUTER JOIN CTSTestSheet_item AS item_error
...
FROM CTSTestSheet
 LEFT OUTER JOIN CTSTestSheet_item AS item_fixture
  ON (item_fixture.report=CTSTestSheet.id AND
      item_fixture.name='fixture')
 LEFT OUTER JOIN CTSTestSheet_item AS item_status
  ON (item_status.report=CTSTestSheet.id AND
      item_status.item=item_fixture.item AND
      item_status.name='status')
 LEFT OUTER JOIN CTSTestSheet_item AS item_name
  ON (item_name.report=CTSTestSheet.id AND
      item_name.item=item_fixture.item AND
      item_name.name='name')
 LEFT OUTER JOIN CTSTestSheet_item AS item_traceback
  ON (item_traceback.report=CTSTestSheet.id AND
