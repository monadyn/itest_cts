...
class ReportTestCase(BaseModelTestCase):
    ...
    def test_delete(self):
        ...
        report_id = db.get_last_id(cursor, 'CTSTestSheet')
        cursor.executemany("INSERT INTO CTSTestSheet_item "
    ...
    def test_insert(self):
        ...
        cursor.execute("SELECT build,step,category,generator "
                       "FROM CTSTestSheet WHERE id=%s", (report.id,))
        self.assertEqual((1, 'test', 'test', 'unittest'),
                         cursor.fetchone())
        cursor.execute("SELECT item,name,value FROM CTSTestSheet_item "
    ...
    def test_insert_empty_items(self):
        ...
        self.assertEqual((1, 'test', 'test', 'unittest'),
                         cursor.fetchone())
        cursor.execute("SELECT COUNT(*) FROM CTSTestSheet_item "
    ...
    def test_select(self):
        ...
        cursor.execute("INSERT INTO CTSTestSheet "
                       "(build,step,category,generator) VALUES (%s,%s,%s,%s)",
                       (1, 'test', 'test', 'unittest'))
        report1_id = db.get_last_id(cursor, 'CTSTestSheet')
        cursor.execute("INSERT INTO CTSTestSheet "
                       "(build,step,category,generator) VALUES (%s,%s,%s,%s)",
                       (1, 'test', 'coverage', 'trace'))
        report2_id = db.get_last_id(cursor, 'CTSTestSheet')
        cursor.executemany("INSERT INTO CTSTestSheet_item "
                           "(report,item,name,value) VALUES (%s,%s,%s,%s)",
